...
const userController = {
  ...
  signUp: async (request, response) => {
    try {
      ...
      console.log(
        "\n>>> je suis de retour dans le controller voici ce que je reçois",
        user
      );
      //access token
      const accessToken = jwt.makeToken(user.id);
      ...
      //mise en place des token et envoi des infos au front
      response
        .header({ Authorization: accessToken, refreshToken: refreshToken })
        .send({ accessToken: accessToken, refreshToken: refreshToken, user });
      console.log(
        "pas d'erreur, je renvoie les tokens et les infos user : ",
        user
      );
    } catch (error) {
      //lire l'erreur
      console.log(
        "!!! Voici l'erreur dans le catch du controller: ",
        error.message
      );
      //console.trace(error);
      //envoyer l'info au front
  ...
  update: async (request, response) => {
    try {
      console.log(
        "updateController >> 1) je vais créer un objet userDatas qui va mixer les données en param et dans le body"
      );
      let userDatas = {
        id: request.params.id,
      };
      console.log(
        "voici ce qui envoyé dans le body par le client",
        request.body
      );
      ...
      // UPDATE
      console.log(
        "updateController >> j'envoie cet objet au model pour qu'il cause avec la DB"
      );
      console.log("- - - - - - -");
  ...
  getUserWithBookmarksInfo: async (request, response) => {
    try {
      ...
      console.log(
        "<<< de retour dans le controller, voici ce que la requête a retourné : "
      );
      console.log(user);
